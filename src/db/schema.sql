...
-- 🆕 এন্ট্রি ১১৭ — সন্দেহভাজন নতুন কারণ: লগে দেখা গেছে বুটের একটা নির্দিষ্ট
-- মুহূর্তে ১৫-২০টা ভিন্ন কোয়েরি (ইন্টারঅ্যাক্টিভ+ব্যাকগ্রাউন্ড, priority-নির্বিশেষে)
-- সবাই প্রায় একই wall-clock মুহূর্তে শেষ হচ্ছে — এটা priority queue দিয়ে
-- ব্যাখ্যা হয় না (priority queue শুধু *কোনটা আগে ডিসপ্যাচ হবে* ঠিক করে, কিন্তু
-- সবাই ক্লাস্টার্ড হয়ে একসাথে "মুক্তি" পাওয়াটা বরং একটা শেয়ার্ড ব্লকিং লক/ইভেন্টের
-- লক্ষণ) — সম্ভাব্য কারণ: dual-write background write (ইনভয়েস/স্টক আপডেট)-এর
-- ফলে wal_autocheckpoint ডিফল্ট থ্রেশহোল্ড (১০০০ পেজ) পার হয়ে একটা checkpoint
-- ট্রিগার হচ্ছে, যেটা সব reader-কে ব্লক করে রাখে যতক্ষণ না checkpoint শেষ হয়
-- (Android-এর ধীর flash storage-এ এটা কয়েক সেকেন্ড লাগতে পারে)। এই দুটো
-- PRAGMA ট্রেড-অফ কমানোর চেষ্টা করছে (ডেটা-নিরাপত্তা অপরিবর্তিত, শুধু
-- checkpoint-এর *সময়* বদলাচ্ছে):
PRAGMA wal_autocheckpoint = 4000;  -- ডিফল্ট ১০০০ পেজ থেকে বাড়িয়ে ৪০০০ — কম ঘনঘন checkpoint ট্রিগার হবে, তাই ইন্টারঅ্যাক্টিভ কোয়েরির মাঝে checkpoint-স্টল কম হওয়ার কথা (WAL ফাইল সাময়িক বড় থাকবে, কিন্তু ডেটা-সেফটিতে কোনো আপস নেই — এখনো crash-safe)
PRAGMA busy_timeout = 3000;        -- আগে সেট করা ছিল না (প্ল্যাটফর্ম-ডিফল্ট অনির্ধারিত আচরণে নির্ভর করছিল) — এখন এক্সপ্লিসিটলি ৩ সেকেন্ড: কোনো lock/checkpoint চলাকালীন কোয়েরি এলে সাইলেন্টলি max ৩সে অপেক্ষা করে রিট্রাই করবে (SQLITE_BUSY এরর ছোঁড়ার বদলে), আচরণ প্রেডিক্টেবল ও ডিবাগযোগ্য হলো
