...
### ৩০ জুলাই ২০২৬ — Session D: ক্যাশ ড্রয়ার ড্যাশবোর্ড কার্ড, উইথড্রয়াল-মোডাল ক্লিনআপ, ফর্ম-রিসেট বাগ ফিক্স

**কেন:** দোকানদার (মোবাইল থেকে স্ক্রিনশট দিয়ে) তিনটা অনুরোধ/সমস্যা জানিয়েছেন:
১. ড্যাশবোর্ডের "ক্যাশ ড্রয়ার" সেকশনে বাটনগুলোর উপরে আজকের বর্তমান ক্যাশ-ড্রয়ার
   ব্যালেন্স (সবাই দেখতে পারবে এমন) দেখানোর কার্ড নেই, আর "হিস্ট্রি" পেজে শুধু
   ওপেনিং/উইথড্রয়াল দেখা যায়, ক্যাশ ড্রয়ারের প্রকৃত ব্যালেন্স না। ২. "ক্যাশ
   উইথড্র করুন" পেজে "পণ্য ফেরত রিফান্ড (নগদ)" ও "ভয়েড রিভার্সাল" — এই দুটো
   ম্যানুয়ালি সিলেক্ট করার মতো বাটন হিসেবে দেখানো হচ্ছিল, অথচ এই দুই টাইপ শুধু
   রিটার্ন/ভয়েড লজিক থেকে অটোমেটিক তৈরি হওয়ার কথা, ম্যানুয়াল এন্ট্রির জন্য না।
   ৩. উইথড্রয়াল মোডালের "সাপ্লায়ারকে দেওয়া"/"অন্যান্য" ফিল্ড আগের এন্ট্রি মনে
   রেখে দিচ্ছিল — মোডাল বন্ধ করে আবার খুললে পুরনো নাম/টাকা থেকে যেত।

**কী কী করা হলো (শুধু `src/App.jsx` ছোঁয়া হয়েছে):**
1. `buildDailySummaryData()`-এ ঐচ্ছিক `dateKeyStart`/`dateKeyEnd` প্যারামিটার
   যোগ হলো (ডিফল্ট না দিলে আগের মতোই শুধু "আজ" — বিদ্যমান ৩টা কল-সাইট
   (নোটিফিকেশন) অপরিবর্তিত থাকবে, কারণ default রেঞ্জ ভেঙে single-day
   equality-এর সমতুল্য)। এখন এই একই শেয়ার্ড ফাংশন/`calcCashDrawer()` cash
   history পেজেও (যেকোনো দিন/মাস রেঞ্জ) ব্যবহার হচ্ছে — আলাদা ডুপ্লিকেট
   ফর্মুলা লেখা হয়নি (CLAUDE.md-এর নির্দেশনা অনুযায়ী)।
2. `Dashboard` কম্পোনেন্টে `todayCashDrawerNow` হিসাব যোগ হলো (একই
   `calcCashDrawer()` দিয়ে) এবং "ক্যাশ ড্রয়ার" সেকশনে ওপেনিং/উইথড্র বাটনের
   উপরে নতুন "আজকের ক্যাশ ড্রয়ার" কার্ড (এডমিন+স্টাফ সবাই দেখতে পাবে, আলাদা
   কোনো রোল-গার্ড যোগ করা হয়নি — বাকি সেকশনের মতোই)।
3. Cash History মোডালে তৃতীয় কার্ড ("আজকের ক্যাশ ড্রয়ার" / মাস-মোডে "ক্যাশ
   ড্রয়ার") যোগ হলো, ওপেনিং/উইথড্রয়াল কার্ডের পাশে (গ্রিড ২→৩ কলাম) এবং
   Print/WhatsApp সামারি HTML-এও একই যোগ হলো।
4. উইথড্রয়াল মোডালের `CASH_TYPE_META` রেন্ডার-লুপ থেকে `return`/
   `return_reversal` ফিল্টার করে বাদ দেওয়া হলো — এই দুটো এখন আর ম্যানুয়াল
   বাটন হিসেবে দেখা যায় না (`CASH_TYPE_META`-তে ডিক্ট এন্ট্রি দুটো এখনও আছে,
   কারণ history-তে label/icon lookup-এর জন্য লাগে)।
5. `Dashboard`-এ নতুন `React.useEffect` যোগ হলো যেটা `cashModal` বন্ধ হওয়ার
   *যেকোনো* পথে (বাতিল বাটন/ফিরুন লিংক/ফোনের হার্ডওয়্যার ব্যাক —
   `useBackHandler(!!cashModal, ...)`) `cashAmount`/`cashNote`/`cashParty`/
   `cashType`/`cashPartyOpen` রিসেট করে দেয়। আগে শুধু বাতিল/ফিরুন বাটনের
   ইনলাইন রিসেট ছিল — হার্ডওয়্যার ব্যাক এই রিসেট মিস করত, যেটাই আসল
   root cause ছিল "সাপ্লায়ারকে দেওয়া/অন্যান্য ফিল্ড পুরনো এন্ট্রি মনে রাখছে" বাগের।

**ইনভয়েস মডিউল প্রসঙ্গে (অসম্পূর্ণ, ব্যবহারকারীর কাছ থেকে ক্ল্যারিফিকেশন লাগবে):**
ব্যবহারকারী বলেছেন ইনভয়েস মডিউলও পূর্বের ইতিহাস মনে রাখছে। কোড-রিভিউ করে
দেখা গেছে `setTab()` ইতিমধ্যে "invoice" ট্যাব থেকে বের হওয়ার সময় `invoiceKey`
বাড়িয়ে `MemoSmartInvoiceBuilder`-কে জোর করে রিমাউন্ট করায় (বিদ্যমান ফিক্স,
২৫ জুলাই ২০২৬) এবং প্রিন্টের পর `resetAll()` সব ফিল্ড খালি করে — অর্থাৎ
বটম-ন্যাভ দিয়ে ট্যাব বদলে আবার "ইনভয়েস"-এ ঢুকলে কার্ট/কাস্টমার/নোট সব খালি
থাকার কথা। কোড-রিভিউ করে কোনো নির্দিষ্ট পাথ পাওয়া যায়নি যেখানে এটা মিস হয় —
তাই এই অংশে **কোনো কোড পরিবর্তন করা হয়নি** (অনিশ্চিত জায়গায় গেস করে
ইনভয়েস/ভয়েড লজিক (প্রায়োরিটি এলাকা #৮) ছোঁয়া ঝুঁকিপূর্ণ)। পরের সেশনে
ব্যবহারকারীর কাছ থেকে সুনির্দিষ্ট রিপ্রো (ঠিক কোন স্টেপ/ফিল্ড, কীভাবে ব্যাক
করা হয়েছিল — বটম-ন্যাভ ট্যাব ক্লিক নাকি ফোনের হার্ডওয়্যার ব্যাক) নিয়ে এটা
আবার দেখা দরকার।

**এর ফলে কী কী পরিবর্তন হলো / প্রায়োরিটি-এলাকা:** এলাকা #৫ (সব হিসাব — cash
drawer)। `calcCashDrawer()`/logic.js ছোঁয়া হয়নি, শুধু App.jsx-এর
`buildDailySummaryData()` (নোটিফিকেশন + এখন হিস্ট্রি উভয়ে ব্যবহৃত) জেনারেলাইজ
করা হয়েছে ঐচ্ছিক প্যারামিটার দিয়ে, ডিফল্ট behavior অপরিবর্তিত। উইথড্রয়াল
সাবট্র্যাকশন ফর্মুলা (owner/supplier/expense/other সবই ইতিমধ্যে
`withdrawalToday`-তে ধরা পড়ে ও ক্যাশ-ড্রয়ার থেকে বিয়োগ হয়) — এটা আগে থেকেই
ঠিক ছিল, নতুন করে কিছু বদলানো হয়নি (যাচাই করে দেখা হয়েছে, শুধু ম্যানুয়াল-বাটন
UI থেকে return/return_reversal সরানো হয়েছে)।

**ভবিষ্যতে মাথায় রাখতে হবে:** (ক) `buildDailySummaryData()`-এর
`dateKeyStart`/`dateKeyEnd` রেঞ্জ শুধু windowed লোকাল `cashLogsAll`/local
`invoices`/`txns` prop থেকে হিসাব করে — মাস-মোডে পুরনো ডেটা (৩৫ দিনের windowed
sync-এর বাইরে) হলে Dashboard prop-এ যতটুকু লোকাল ডেটা আছে ততটুকুই ধরবে (এটা
আগে থেকেই histOpeningTotal/histWithdrawTotal-এর ক্ষেত্রেও একই সীমাবদ্ধতা,
নতুন কিছু না)। (খ) `CASH_TYPE_META`-তে নতুন কোনো টাইপ যোগ করলে সেটা
ম্যানুয়াল-বাটন গ্রিডে দেখাতে চান কিনা ভেবে দেখে filter লিস্টে (`return`,
`return_reversal`) যোগ করতে হবে কিনা ঠিক করুন। (গ) ইনভয়েস মডিউল সংক্রান্ত
অংশ এখনো অসমাধিত/অস্পষ্ট — উপরে দেখুন।

**কনসিকুয়েন্স/যাচাই:** `npm test`-এর মধ্যে `logic-tests.mjs` (68✅),
`integration-tests.mjs` (10✅), `sync-tests.mjs` (24✅) — এই sandbox-এ চালানো
হয়েছে ও পাস করেছে (logic.js/sync.js কোনোটাই এই সেশনে ছোঁয়া হয়নি, তাই এগুলো
শুধু বেসলাইন-রিগ্রেশন-চেক)। `schema-tests.mjs` ও `test:fuzz` এই sandbox-এ
নেটওয়ার্ক না থাকায় চালানো যায়নি (`zod`/`fast-check` npm প্যাকেজ ইনস্টল করা
নেই) — CI-তে এগুলো চালিয়ে নিশ্চিত হওয়া বাকি, তবে যেহেতু `src/logic.js`/
`src/schemas.js` কোনোটাই এই সেশনে বদলানো হয়নি, ঝুঁকি কম। `src/App.jsx`-এর
সম্পূর্ণ JSX সিনট্যাক্স esbuild দিয়ে কম্পাইল করে যাচাই করা হয়েছে (কোনো এরর
নেই) — এটা শুধু সিনট্যাক্স-লেভেল যাচাই, রানটাইম আচরণ (React DevTools/আসল
ডিভাইসে) ম্যানুয়ালি টেস্ট করা হয়নি, কারণ এই sandbox-এ অ্যাপ রান করে UI
ইন্টারঅ্যাক্ট করার সুযোগ নেই। ব্যবহারকারীর ডিভাইসে বসানোর পর অন্তত এই তিনটা
যাচাই করে নেওয়া উচিত: (১) ওপেনিং/উইথড্রয়াল দেওয়ার পর "আজকের ক্যাশ ড্রয়ার"
কার্ডের সংখ্যা সঠিক কিনা, (২) হিস্ট্রি পেজে পুরনো দিন/মাস সিলেক্ট করলে তিনটা
কার্ডই ঠিকঠাক দেখাচ্ছে কিনা, (৩) উইথড্রয়াল মোডাল থেকে হার্ডওয়্যার ব্যাক
চেপে বের হয়ে আবার ঢুকলে ফিল্ড সত্যিই খালি আসছে কিনা।
