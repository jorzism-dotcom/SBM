...
## 🗺️ Phase ৩ (read-path cutover / বুট-চেঞ্জ) রোডম্যাপ — সিরিয়াল অনুযায়ী (এন্ট্রি ৯৩-এ তৈরি)

> এস্টিমেট আনুমানিক — products migration (এন্ট্রি ৪৪-৯২, ~১৫টা ডেডিকেটেড এন্ট্রি) থেকে প্যাটার্ন
> ধরে করা, বাস্তবে কম-বেশি হতে পারে। **প্রতিটা ধাপ শেষে এই তালিকায় ✅ মার্ক করে দিতে হবে** এবং
...
**C. চূড়ান্ত ধাপ (A ও B দুটোই শেষ হওয়ার পরে)**
9. ⬜ CRITICAL_KEYS থেকে customers/invoices আসলে সরানো — flag-controlled canary rollout, প্রথমে শুধু ব্যবহারকারীর ফার্মেসিতে *(২-৩ সেশন)*
10. ⬜ ৪-৬ সপ্তাহ প্রোডাকশন-স্টেবিলিটি প্রমাণ (কোনো কোড কাজ না, শুধু অপেক্ষা+মনিটরিং) — তারপরই Phase 5 (পুরনো IndexedDB blob-array কোড অপসারণ) *(১-২ সেশন)*

**মোট আনুমানিক**: ~২০-৩০টা সেশন। ধাপ ৭ (invoice numbering) সবচেয়ে বড় ঝুঁকি — এখানে কোনো শর্টকাট নেওয়া যাবে না।
...
- `tests/datastore-analytics-parity-tests.mjs` — **নতুন** ফাইল (১১ কেস)
...
**উপসর্গ (real-device স্ক্রিনশট)**: অ্যাপ খোলার পর পণ্য তালিকা ("০টি") আর কাস্টমার তালিকা ("০জন") দুটোই ~৫ সেকেন্ড খালি দেখায়, তারপর ডেটা আসে। Splash স্ক্রিন ততক্ষণে আগেই সরে গেছে (boot log-এ "প্রথম _patch সম্পন্ন" ~২০০-৩০০ms-এই)।

**রুট কজ**: এন্ট্রি ১১৬-এর boot-grace priority queue শুধু *queue-তে অপেক্ষমাণ* আইটেমগুলোর মধ্যে ইন্টারঅ্যাক্টিভ কলকে আগে বসাতে পারে — কিন্তু যেটা ইতিমধ্যে native bridge-এ dispatch হয়ে গেছে (এই মুহূর্তে এক্সিকিউট হচ্ছে) সেটাকে থামানো/রিঅর্ডার করা অসম্ভব (bridge সম্পূর্ণ সিরিয়াল, একবারে একটাই কল)। কোল্ড বুটে products/customers-list-screen-এর `queryPage()` queue-তে পৌঁছানোর *আগেই* (React মাউন্ট effect চলার আগে) queue তখনো খালি থাকা অবস্থায় প্রথম ব্যাকগ্রাউন্ড (untagged) KPI কল dispatch হয়ে যেতে পারে — আর কোল্ড page-cache-এ একটা ব্যাকগ্রাউন্ড অ্যাগ্রিগেট কলের নেটিভ খরচই লগে বারবার ৩৯০০-৪৩০০ms দেখা গেছে (উদাহরণ: শুধু ১৮ রো-র customers COUNT+SUM কল)। পুরো এই সময়টা ইন্টারঅ্যাক্টিভ কলও পেছনে সিরিয়ালাইজড হয়ে আটকে থাকে — এটাই স্ক্রিনশটের ৫ সেকেন্ড খালি-তালিকার আসল কারণ।

**ফিক্স**: `src/db/DataStore.js`-এর `_pumpDbQueryQueue()`-এ অ্যাপের জীবনে *প্রথমবার* pump হওয়ার সময় একটা ছোট্ট settle-window (`INITIAL_SETTLE_MS = 250`, বা তার আগেই queue-তে কোনো ইন্টারঅ্যাক্টিভ কল এলে সাথে সাথে বেরিয়ে) যোগ হলো — এতটুকু সময়ে React-এর মাউন্ট effect queue-তে পৌঁছানোর সুযোগ পায়, তাই প্রথম dispatch-টাই সবসময় ইন্টারঅ্যাক্টিভ কল হয়, কোনো ব্যাকগ্রাউন্ড কল না। এটা `_firstPumpSettled` ফ্ল্যাগ দিয়ে শুধু কোল্ড-বুটে *একবারই* প্রযোজ্য — পরের প্রতিটা pump আগের মতোই তাৎক্ষণিক, রানটাইম ওভারহেড নেই।

**যাচাই (sandbox)**: isolated esbuild দিয়ে `DataStore.js` সিনট্যাক্স-ভ্যালিডেট — `SYNTAX OK`। sandbox-এ `npm run build`/ডিভাইস টেস্ট চালানো যায়নি (আগের এন্ট্রিগুলোর মতোই সীমাবদ্ধতা) — পুশের পর CI + real-device দিয়ে কনফার্ম করতে হবে।

**real-device-এ এখন যা করবেন**: নতুন বিল্ড ইনস্টল করে অ্যাপ কোল্ড-স্টার্ট করুন (ফোর্স-স্টপ করে আবার খুলুন), সাথে সাথে পণ্য/কাস্টমার ট্যাবে যান — এবার তালিকা প্রায় সাথে সাথেই আসা উচিত, ৫ সেকেন্ড খালি না দেখানো উচিত। টাইমিং ডায়াগনস্টিক লগও পাঠাবেন — `queryPage-EXPLAIN:products`/`customers` ট্যাগের `queue-wait` এখন খুব কম (< কয়েকশ ms) হওয়া উচিত বুটের প্রথম দিকে।

**📁 এই সেশনে যেসব ফাইল বদলেছে**: `src/db/DataStore.js` (`_pumpDbQueryQueue()`-এ initial-settle লজিক), `SQLITE_MIGRATION_LOG.md` (এই এন্ট্রি)

---

## 🎯 আগের মাস্টার স্ট্যাটাস (এন্ট্রি ১২১-এ আপডেট)

**✅ এন্ট্রি ১২১ — চূড়ান্ত root cause ধরা পড়েছে ও ফিক্স করা হয়েছে: redundant concurrent getAllRows(products) ব্যাকআপ কল, disk-throttling/battery/write-lock কোনোটাই না**

**⚠️ এন্ট্রি ১২০-এর "write lock" সন্দেহ ভুল প্রমাণিত — বাতিল করা হলো।** এন্ট্রি ১২০-এ যোগ করা `✍️` write-tracking দিয়ে পরের real-device লগে **একটাও write (`db.run`/`db.execute`) কল দেখা যায়নি** — write-lock থিওরি খারিজ। এরপর লগের boot-relative timestamp-এ অস্বাভাবিক লাফ (`boot+2014499ms`≈৩৩.৫মি, `boot+1112053ms`≈১৮.৫মি) নিয়ে সন্দেহ হয়েছিল app-lifecycle/OS-level disk-throttling — কিন্তু কোড অডিট করে দেখা গেল আসল কারণ অনেক সরল ও কোড-লেভেলেই ধরা পড়ে।

**রুট কজ**: `App.jsx`-এর ব্যাকআপ `tick()`/`cycle()` ফাংশন `runLocalBackup()`, `runDriveBackup()`, `runSnapshotBackup()` — তিনটাকেই **একসাথে, await ছাড়া পরপর** কল করে। এই তিনটাই আলাদাভাবে `buildBackupData()` (বা তার latest-ref) কল করে, আর `buildBackupData()`-এর ভেতরেই (এন্ট্রি ৭৫) `dsGetAllRows(businessType, "products")` — পুরো ২২৩৭-রেকর্ড টেবিল স্ক্যান — সরাসরি কল হয়, কোনো শেয়ারিং/ডিডুপ ছাড়াই। ফলে **একটা tick-এই ৩টা সমান্তরাল ফুল-টেবিল-স্ক্যান**। `tick()`-কে ডাকার একাধিক ট্রিগার আছে (`visibilitychange`, Capacitor `resume`, `foreground-safety` টাইমার) — অ্যাপ রিজিউমে দুইটা ইভেন্ট প্রায় একই মুহূর্তে ফায়ার করলে ২× tick = ৬টা পর্যন্ত সমান্তরাল স্ক্যান হতে পারে।

**প্রমাণ (দুটো ভিন্ন real-device লগে কনফার্মড)**:
- `boot+2014499ms`/`boot+1112053ms`-এর কাছাকাছি `শুরুতে in-flight=5` — মানে ওই মুহূর্তে আরও ৫টা কোয়েরি আগে থেকেই সমান্তরালে চলছিল।
- আরেকটা সেশনে `boot+740088ms`→`boot+740214ms`-এর মধ্যে (মাত্র ১৩০ms-এর ভেতর) **৪টা আলাদা `getAllRows(products) ব্রেকডাউন` সমাপ্তি** — প্রতিটাই পূর্ণাঙ্গ ২২৩৭-রেকর্ড স্ক্যান, একে অপরের সাথে সমান্তরালে। একই সেশনে আরও ৩ জায়গায় (boot+31s, +3মি, +4.7মি) একই প্যাটার্ন, backup ইন্টারভালের সাথে সামঞ্জস্যপূর্ণ ব্যবধানে।

**কী করা হলো (ফিক্স)**: `src/db/DataStore.js`-এর `getAllRows()`-এ `getDb()`-এর in-flight promise cache-এর (এন্ট্রি ৫৭) ঠিক একই প্যাটার্নে single-flight deduplication যোগ হলো — কোনো (businessType, store) জোড়ার জন্য একটা `getAllRows()` ইতিমধ্যে চলমান থাকলে, নতুন কল আলাদা DB স্ক্যান শুরু না করে সেই একই in-flight promise-এই await করবে। Resolve/reject হওয়ামাত্র cache থেকে entry সরে যায় (`finally`), তাই স্টেলনেস নেই — শুধু "একই মুহূর্তে একাধিক কলার" ডিডুপ হয়, পরের tick-এ (৫+ মিনিট পরে) স্বাভাবিক ফ্রেশ স্ক্যান হবে। এটা `buildBackupData()`/`buildManualBackupData()`/`performMasterSync()` — সব কলারকে একসাথে কভার করে, `App.jsx`-এর `tick()` লজিক স্পর্শ না করেই (সবচেয়ে কম-ঝুঁকির ফিক্স পয়েন্ট, যেহেতু এটা লাইভ প্রোডাক্টে)।

**যাচাই (sandbox)**: isolated esbuild দিয়ে `DataStore.js` সিনট্যাক্স-ভ্যালিডেট — `SYNTAX OK`। এই সেশনে sandbox-এ `npm test`/`npm run build` চালানো যায়নি (network/peer-dependency সীমাবদ্ধতা, আগের এন্ট্রিগুলোর মতোই) — পুশের পর CI (`build-apk.yml`) দিয়ে কনফার্ম করা উচিত।

**real-device-এ এখন যা করবেন**: নতুন বিল্ড ইনস্টল করে, অ্যাপ কিছুক্ষণ খোলা রেখে ব্যাকগ্রাউন্ডে নিয়ে যান (৫+ মিনিট), ফিরে এসে টাইমিং ডায়াগনস্টিক লগ কপি করে পাঠান — এবার `getAllRows(products) ব্রেকডাউন` লাইনগুলো প্রতি tick-এ একবারই (একাধিক নয়) আসা উচিত, আর সেই সময়ের আশেপাশের jank/queue-wait অনেক কম হওয়া উচিত।

**যা এখনো করা যেতে পারে (ভবিষ্যতে, অপশনাল)**: `tick()`-এ `runLocalBackup`/`runDriveBackup`/`runSnapshotBackup`-এর মধ্যে products fetch ছাড়াও অন্যান্য ওভারল্যাপ (যেমন `performMasterSync()` নিজের `dsGetAllRows` কলও এখন একই cache-এ ডিডুপ হচ্ছে, কিন্তু `visibilitychange`+`capacitor-resume` ডাবল-tick এখনো কোড-লেভেলে ঠিক করা হয়নি — `getAllRows()`-এর ফিক্সেই কার্যত সমাধান হয়ে গেছে বলে এখন প্রায়োরিটি কম)।

**📁 এই সেশনে যেসব ফাইল বদলেছে**:
- `src/db/DataStore.js` — `getAllRows()`-এ single-flight in-flight promise cache (`_getAllRowsPromiseCache`) যোগ, আসল লজিক নতুন internal `_getAllRowsRun()`-এ সরানো হলো
- `SQLITE_MIGRATION_LOG.md` — এই এন্ট্রি (১২১), এন্ট্রি ১২০-এর write-lock সন্দেহ বাতিল

---

## 🎯 আগের মাস্টার স্ট্যাটাস (এন্ট্রি ১২০-এ আপডেট)

**🎯 এন্ট্রি ১২০ — সম্ভাব্য আসল কারণ ধরা পড়েছে: write lock, ব্যাটারি/থ্রটলিং না**

এন্ট্রি ১১৯-এর queue-wait/native-exec ভাঙন দিয়ে প্রথমবার একটা সরাসরি প্রমাণ পাওয়া গেছে: একটা কল (`SELECT COUNT(*)... FROM customers`, মাত্র ১৮ রো, ইনডেক্স-সহ) **queue-wait মাত্র ৬৫ms ছিল কিন্তু তার নিজের নেটিভ এক্সিকিউশনই ৪২৬৬ms নিয়েছে** — এটা queueing/ইনডেক্স সমস্যা না, `busy_timeout=3000ms`-এর কাছাকাছি সংখ্যা হওয়ায় সন্দেহ: **কোনো সমান্তরাল write ট্রানজেকশন লক ধরে রেখেছিল**।

কিন্তু এতদিন `db.run()`/`db.execute()` (write অপারেশন — upsert, delete, FTS sync, schema/ALTER, ANALYZE) সম্পূর্ণ untracked ছিল, শুধু `db.query()` (read) ট্র্যাক হতো। তাই কোন write ঠিক ওই মুহূর্তে চলছিল তা লগে দেখাই যেত না। **এই সেশনে `db.run()`/`db.execute()`-ও একই queue+timing ব্যবস্থায় যোগ হলো** (`✍️` আইকনে, read থেকে আলাদা)।

**পরের লগেই দেখা উচিত**: যদি সত্যিই write lock-ই কারণ হয়, তাহলে ধীর read কলের ঠিক আশেপাশেই একটা `✍️ [db.run/execute...]` লাইন দেখা যাবে বড় নেটিভ-exec সহ — সেটাই আসল দোষী। যদি কোনো write-ই না দেখা যায়, তাহলে ধারণাটাও ভুল, আরও গভীরে যেতে হবে (SQLite-এর নিজস্ব internal lock/mutex, বা প্লাগইন-লেভেল bridge সমস্যা)।

**⚠️ এন্ট্রি ১১৮-এর "battery-throttling" উপসংহার ভুল ছিল, প্রত্যাহার করা হয়েছে** (বিস্তারিত এন্ট্রি ১১৯-এ, নিচে)। ব্যবহারকারী স্পষ্ট নিশ্চিত করেছেন এটা ব্যাটারি/থার্মাল/ব্যাকগ্রাউন্ড-অ্যাপ কোনোটাই না।

**📁 এই সেশনে বদলেছে**: `src/db/DataStore.js` (db.run/db.execute-এর জন্য queue-wait/native-exec ট্র্যাকিং যোগ), `SQLITE_MIGRATION_LOG.md`।

---

## 🎯 আগের মাস্টার স্ট্যাটাস (এন্ট্রি ১১৯-এ আপডেট)

**⚠️ এন্ট্রি ১১৮-এর সংশোধনী — "battery-throttling" উপসংহারটা ভুল ছিল, লগ থেকে বাতিল করা হলো**

এন্ট্রি ১১৮-এ লেখা হয়েছিল যে ব্যবহারকারী চার্জে-লাগানো অবস্থায় টেস্ট করে "কোনো দেরি নেই" কনফার্ম করেছেন — কিন্তু এটা ভুল বোঝাবুঝি ছিল, ব্যবহারকারী শুধু বলেছিলেন "চার্জে সমস্যা নেই" (অর্থাৎ চার্জ দিতে তার আপত্তি নেই/সমস্যা নেই), **প্রকৃত চার্জ-অবস্থায় টেস্ট করে ফলাফল কনফার্ম করেননি**। ব্যবহারকারী স্পষ্টভাবে জানিয়েছেন এটা ব্যাটারি/থার্মাল/ব্যাকগ্রাউন্ড-অ্যাপ কোনোটাই না। তাই এন্ট্রি ১১৮-এর "root cause confirmed: Android battery-saver throttling" — এই উপসংহার **প্রত্যাহার করা হলো**। প্রকৃত কারণ এখনো অজানা, তদন্ত চলছে (নিচে দেখুন)।

**যা এখনো সত্যি/বৈধ (এন্ট্রি ১১৮-এর বাকি অংশ)**: SQL ইনডেক্স ফিক্স (এন্ট্রি ১১০-১১১), priority queue (এন্ট্রি ১১৩-১১৬), লোডিং স্পিনার — এগুলো আসল, কার্যকর ফিক্স ছিল, বহাল আছে। WAL checkpoint বাতিল হয়েছে লগ-প্রমাণ দিয়ে, সেটাও বহাল থাকছে।

**🔍 নতুন সন্দেহ (এন্ট্রি ১১৯) — background dashboard কোয়েরিগুলোর নিজস্ব SQL প্ল্যান কখনো যাচাই করা হয়নি**

এতদিন ধরে নেওয়া হয়েছিল background কোয়েরির (`dosage_form`, `supplier_due`, `nearest_expiry_date`, `stock` রেঞ্জ/ORDER BY, `sold30` অ্যাগ্রিগেট ইত্যাদি) বড় "লাগলো Xms" সংখ্যাগুলো **queue-wait** (অন্য কোয়েরির পেছনে অপেক্ষা), কারণ শুধু products/customers browse কলের জন্যই native db.query() breakdown মাপা হয়েছিল, যেটা সবসময় দ্রুত দেখাচ্ছিল। কিন্তু **background কোয়েরিগুলোর নিজস্ব native execution time কখনো আলাদাভাবে মাপাই হয়নি** — সম্ভবত সেগুলোর নিজেরই কোনো ইনডেক্স নেই (browse-এর জন্য যেই ইনডেক্স ফিক্স করা হয়েছিল, সেটা শুধু browse-এর জন্যই ছিল), আর ২২৩৭টা প্রোডাক্টে ১০+টা ভিন্ন full-scan/sort/DISTINCT কোয়েরি সিরিয়ালি চললে প্রতিটাই কয়েকশো ms করে নিলে যোগফল সত্যিই কয়েক সেকেন্ড হতে পারে — battery/thermal ছাড়াই, শুধু মিসিং ইনডেক্সের কারণে।

এই সেশনে `_pumpDbQueryQueue()`-এ **queue-wait বনাম native-exec আলাদা করে** মাপা শুরু হয়েছে (সব কল-এর জন্য, শুধু browse না), আর ধীর/ব্যাকগ্রাউন্ড কলের জন্য `EXPLAIN QUERY PLAN` ক্যাপচার যোগ হয়েছে — পরের লগেই সরাসরি দেখা যাবে এই ~১০টা background কোয়েরির মধ্যে কোনটা SCAN করছে (ইনডেক্স নেই) বনাম SEEK করছে, আর queue-wait বনাম আসল-execution-এর অনুপাত।

**📁 এই সেশনে বদলেছে**: `src/db/DataStore.js` (queue-wait/native-exec breakdown + ব্যাকগ্রাউন্ড কলের EXPLAIN), `SQLITE_MIGRATION_LOG.md` (এই এন্ট্রি + এন্ট্রি ১১৮ সংশোধনী)।

---

## 🎯 আগের মাস্টার স্ট্যাটাস (এন্ট্রি ১১৮-এ আপডেট, উপসংহার আংশিক ভুল — উপরে সংশোধনী দেখুন)

**🔍 এন্ট্রি ১১৮ (Products/Customers কোল্ড-স্টার্ট লেটেন্সি — root cause চূড়ান্তভাবে নিশ্চিত, ব্যবহারকারীর ডিভাইস-টেস্ট দিয়ে) — কোড-লেভেল অপ্টিমাইজেশন শেষ, বাকিটা OS-লেভেল, ফিক্স-অযোগ্য**

**সমস্যা (এন্ট্রি ১০৬-১১৭ জুড়ে ধাপে ধাপে ডায়াগনোজ)**: cold-start-এ Products/Customers তালিকা কখনো তাৎক্ষণিক, কখনো ২-৬ সেকেন্ড ফাঁকা থাকত — অসামঞ্জস্যপূর্ণভাবে, একই বিল্ডে একেক বুটে একেক রকম।

**ধাপে ধাপে যা বাদ দেওয়া হয়েছে (প্রতিটাই real-device লগ দিয়ে যাচাই করে)**:
1. **SQL ইনডেক্স/query-plan সমস্যা** (এন্ট্রি ১১০-১১১) — `demand_type` কলামে OR-প্যাটার্ন ইনডেক্স-সিক ভাঙছিল, ফিক্স হয়েছে (এখন `EXPLAIN QUERY PLAN`-এ সবসময় `SEARCH ... USING INDEX`)। **এটা আসল ফিক্স ছিল, রাখা হয়েছে** — নেটিভ `db.query()` এখন সবসময় <২০ms।
2. **কোল্ড page-cache** (এন্ট্রি ১১২) — warm-up query যোগ হয়েছে, সাহায্য করেছে কিছুটা কিন্তু পুরোপুরি না।
3. **Connection সিরিয়ালাইজেশন/queue-ordering** (এন্ট্রি ১১৩-১১৬) — priority queue + boot-grace period যোগ হয়েছে (Products/Customers browse কল সবসময় ব্যাকগ্রাউন্ড dashboard-অ্যাগ্রিগেটের আগে যায় এখন)। **এটাও রাখা হয়েছে, বাস্তব উন্নতি করে** (charge-এ থাকা অবস্থায় ব্যবহারকারী কনফার্ম করেছেন এখন তাৎক্ষণিক লোড হয়)।
4. **WAL checkpoint স্টল** (এন্ট্রি ১১৭) — `wal_checkpoint(PASSIVE)` স্ট্যাট দিয়ে সরাসরি বাতিল হয়েছে (`log`/`checkpointed` সবসময় সমান, `busy=0`) — এটা কারণ ছিল না।

**✅ চূড়ান্ত root cause: Android battery-saver/Doze CPU থ্রটলিং** — উভয় ধীর টেস্ট-রানেই ব্যাটারি ~১০-২৩% ছিল (স্ক্রিনশটে কনফার্মড), আর ব্যবহারকারী নিজে চার্জে-লাগানো অবস্থায় (৫০%+) টেস্ট করে কনফার্ম করেছেন — **কোনো দেরি নেই**। কম ব্যাটারিতে Android পুরো অ্যাপ প্রসেসের ব্যাকগ্রাউন্ড CPU/I/O থ্রটল করে, যেটা আমাদের JS-সাইড priority queue দিয়ে বাইপাস করা সম্ভব না (ব্লকিং OS scheduler-লেভেলে, আমাদের কোড-লেভেলে না)। **এটা SBM-এর বাগ না — এটা Android-এর স্বাভাবিক পাওয়ার-ম্যানেজমেন্ট আচরণ, প্রতিটা অ্যাপেই হয়।**

**যা তবুও ফিক্স করা হলো (real, actionable UX ইমপ্রুভমেন্ট)**: Products/Customers তালিকার প্রথম-লোড অবস্থায় (SQL browse এখনো ডেটা আনছে) স্ক্রিন আগে সম্পূর্ণ ফাঁকা থাকত — কোনো "খালি" মেসেজও না, কোনো স্পিনারও না, ব্যবহারকারীর কাছে "অ্যাপ আটকে গেছে" মনে হতো। এখন স্পষ্ট লোডিং স্পিনার + "লোড হচ্ছে…" টেক্সট দেখাবে — থ্রটলিং-এর কারণে দেরি হলেও অ্যাপ যে কাজ করছে (আটকে যায়নি) সেটা স্পষ্ট বোঝা যাবে।

**⚠️ যা এখনো কোড-লেভেলে করা হয়নি কিন্তু ভবিষ্যতে সাহায্য করতে পারে**:
- [ ] ব্যাটারি কম থাকলে (Battery Status API দিয়ে চেক করে) ব্যবহারকারীকে একটা ঐচ্ছিক তথ্যমূলক নোট দেখানো ("কম ব্যাটারিতে অ্যাপ কিছুটা ধীর হতে পারে — Android-এর পাওয়ার সেভিং ফিচার")
- [ ] Dashboard-এর ব্যাকগ্রাউন্ড KPI কোয়েরি (সাপ্লায়ার-ডিউ, low-stock, expiry ইত্যাদি) বুটের পরিবর্তে Dashboard ট্যাব আসলে খোলা হলে lazy-trigger করা — এখন এগুলো সবসময় বুটে ফায়ার হয়, priority queue দিয়ে deprioritize করা আছে কিন্তু আদৌ না চালানো (screen ভিজিবল না হলে) আরও ভালো — স্কোপ আউট হয়েছে এই সেশনে, ভবিষ্যতে আলাদা সেশনে করা যেতে পারে

**📁 এই সেশনে যেসব ফাইল বদলেছে**:
- `src/db/schema.sql` — `idx_products_deleted_demand_name_id`/`idx_customers_deleted_updated_id` কভারিং ইনডেক্স, `wal_autocheckpoint=4000`, `busy_timeout=3000`
- `src/db/DataStore.js` — `demand_type` write-time normalize + one-time backfill; cold-start warm-up query; priority queue (`_enqueueDbQuery`/`_pumpDbQueryQueue`) + boot-grace period; কনকারেন্সি ডায়াগনস্টিক (`🧵`); DB স্বাস্থ্য-চেক (`🩺`, WAL checkpoint stat সহ)
- `src/App.jsx` — `browseWhereFor()` OR→equality; `queryPage`/`getByIds` কল-সাইটে diag ট্যাগ; মেইন-থ্রেড jank মনিটর; **Products/Customers তালিকায় লোডিং স্পিনার** (নতুন, এই এন্ট্রিতে)
- `src/db/DiagLog.js` — `MAX_ENTRIES` ৬০→৪০০
- `SQLITE_MIGRATION_LOG.md` — এই এন্ট্রি

---

## 🎯 আগের মাস্টার স্ট্যাটাস (এন্ট্রি ১০৬-এ আপডেট)
...
**প্রেক্ষাপট**: ব্যবহারকারী সরাসরি বড় বুট-চেঞ্জ (customers/invoices পুরো array কখনো লোড না করা) শুরু করতে চাইলেন, ঝুঁকি জেনেই। কোড-অডিটে ধরা পড়ল এই স্কোপ (customers-এ ২৩টা + invoices-এ ৩৫টা ফুল-অ্যারে-স্ক্যান সাইট) products-এর (৫১টা সাইট, এন্ট্রি ৪৯-৯২ জুড়ে বহু সেশন লেগেছে) সাথে তুলনীয়/বড় — এক সেশনে সম্পূর্ণ দায়িত্বশীলভাবে করা সম্ভব না, বিশেষত টাকা-সরাসরি-প্রভাবিত কোডে (কাস্টমার ব্যালেন্স, ইনভয়েস)। তাই এই সেশনে বুট-চেঞ্জের **আসল নিরাপদ প্রথম ভিত্তি** তৈরি করা হলো — products migration নিজেই যেভাবে এন্ট্রি ৭৭-এ প্রথমে merge-patch প্রিরিকুইজিট ফিক্স করে তারপরই এন্ট্রি ৭৮+-এ never-load বানিয়েছিল, ঠিক একই ক্রম অনুসরণ করে।

**🔴 রিয়েল বাগ ধরা পড়েছে ও ফিক্স হয়েছে (এন্ট্রি ৭৭-এর productsById বাগের ঠিক একই ক্লাস)**: `customersById`/`invoicesById` global Map-এর subscriber আগে **wholesale-rebuild** করত (`new Map(customers.map(...))`) প্রতিবার `customers`/`invoices` state বদলালেই — customers/invoices সবসময় বুটে পূর্ণ থাকা বর্তমান অবস্থায় নিরাপদ ছিল। কিন্তু ভবিষ্যতে এই দুটো সত্যিকারভাবে lazy/আংশিক হলে (বুট-চেঞ্জের লক্ষ্য) এই wholesale-rebuild প্রতিটা সাধারণ এডিটেই SQLite থেকে হাইড্রেট করা বাকি সব এন্ট্রি নীরবে মুছে ফেলত — ঠিক productsById-এর সেই একই ডেটা-করাপশন-ক্লাস বাগ। **ফিক্স**: `productsById`-এর প্রমাণিত/টেস্টেড `mergeItemsIntoIdMap()` (logic.js, pure) পুনর্ব্যবহার করে দুটো subscriber-ই merge-patch-এ কনভার্ট করা হলো। **আচরণ এই মুহূর্তে অপরিবর্তিত** (customers/invoices এখনো সবসময় পূর্ণ) — এটা শুধু ভবিষ্যতের জন্য নিরাপদ পূর্বশর্ত।

**Category A কনসোলিডেশন (customers)**: ৬টা বিলিং/UI-সংশ্লিষ্ট `customers.find(c => c.id === X)` সাইট (রিসিট প্রিন্ট, InvoiceVoidModal, Dashboard invoice-view ×২, Customers কম্পোনেন্ট delete-confirm, SmsLog sendCustom, CustomerDetail invoice-view) O(n) থেকে গ্লোবাল `useAppStore.getState().customersById` O(1) lookup-এ কনভার্ট — products-এর এন্ট্রি ৬১-এর ঠিক একই প্যাটার্ন, zero-regression (sync, কোনো নতুন async/SQL involved না)। **ইচ্ছাকৃতভাবে বাদ**: ২টা মোবাইল-বেজড ডুপ্লিকেট-চেক সাইট (`customers.find(c => c.mobile === mobile)`) — আলাদা key, id-Map দিয়ে কনভার্ট করা যায় না, এই পাসের স্কোপের বাইরে।

**⚠️ যা এখনো বাকি, সততার সাথে**:
- **আসল বুট-চেঞ্জ শুরুই হয়নি** — `customers`/`invoices` এখনো CRITICAL_KEYS-এ পুরো array হিসেবেই বুটে সিঙ্ক্রোনাসভাবে লোড হয়, কোনো lazy/never-load ফ্ল্যাগ নেই এখনো
- Category B (aggregate — ৭টা customers সাইট: totalBaki, bakiCustomers, bakiCount/clearCount ইত্যাদি) এখনো JS array-scan, SQL cutover হয়নি
- Category C (full-list render — Customers main list card serial-numbering, RFM লিস্ট) এখনো raw `customers` array থেকেই রেন্ডার হয় — products main list-এর মতো id+hydrate প্যাটার্নে কনভার্ট হয়নি
- Invoices-এর ৩৫টা সাইট এই সেশনে touch করাই হয়নি (শুধু invoicesById subscriber ফিক্স হয়েছে) — invoices-এর স্কেল products-এর চেয়েও বড় (১ কোটি+ টার্গেট) আর প্ল্যান ডকের নিজস্ব যুক্তি অনুযায়ী "natural time cutoff আছে" (৬-মাস windowed), তাই সম্ভবত never-load-এর বদলে windowed-boot ডিজাইন লাগবে — products-এর প্যাটার্ন সরাসরি কপি করা যাবে না
- 2টা মোবাইল-ডুপ্লিকেট-চেক সাইট এখনো O(n) (নিরাপদ কিন্তু অপ্টিমাইজড না)

**যাচাই সম্পূর্ণ (sandbox)**: `npm test` (সব প্রি-এক্সিস্টিং সুইট পাস) → `npm run lint` (0 error, বেসলাইন ৫৬১→৫৬৩ সামান্য বৃদ্ধি — নতুন কমেন্ট লাইনের কারণে, কোনো নতুন unused-var/no-undef না) → `npm run typecheck` (ক্লিন) → `npm run build` (ক্লিন) → `test:golden-master` (৭/৭) → `test:fuzz` (সব প্রপার্টি)। **real-device স্মোক-টেস্ট এখনো বাকি** — রিসিট প্রিন্ট, ইনভয়েস ভয়েড, কাস্টমার ডিলিট, SMS পাঠানো, ড্যাশবোর্ড/CustomerDetail-এ ইনভয়েস-ভিউ (কাস্টমার নাম/মোবাইল ঠিক দেখাচ্ছে কিনা)।

**📁 এই সেশনে যেসব ফাইল বদলেছে**:
- `src/App.jsx` — `customersById`/`invoicesById` subscriber merge-patch-এ কনভার্ট (`mergeItemsIntoIdMap()` পুনর্ব্যবহার); ৬টা `customers.find(id)` সাইট গ্লোবাল `customersById` O(1) lookup-এ কনভার্ট
- `SQLITE_MIGRATION_LOG.md` — এই এন্ট্রি (৯৩) যোগ

**পরের সেশনে করণীয়**:
1. real-device স্মোক-টেস্ট (উপরে তালিকাভুক্ত ৬টা সাইট)
2. Category B (customers aggregate সাইট) SQL cutover ডিজাইন
3. Category C (Customers main list card, RFM) id+hydrate প্যাটার্নে কনভার্ট — products main list card-এর (এন্ট্রি ৫৩) প্যাটার্ন অনুসরণ করে
4. Invoices-এর জন্য আলাদা windowed-boot ডিজাইন (never-load না) — এটা নিজেই একটা পূর্ণ, আলাদা প্ল্যানিং সেশনের কাজ
5. তারপরই আসল CRITICAL_KEYS থেকে customers/invoices সরানো (বুট-চেঞ্জ চূড়ান্ত ধাপ) — সবচেয়ে ঝুঁকিপূর্ণ, flag-controlled canary rollout সহ

---

## 🎯 আগের মাস্টার স্ট্যাটাস (এন্ট্রি ৯২-এ আপডেট — নতুন সেশনে প্রথমে এই সেকশনটাই পড়ুন)

**🟢 এন্ট্রি ৯২ (✅✅✅ সব real-device ভেরিফিকেশন সম্পূর্ণ — কোনো কোড বদল হয়নি এই এন্ট্রিতে, শুধু যাচাই-কনফার্মেশন)**:

**ব্যবহারকারী কনফার্ম করলেন**: "সব চেক ডান। সব পারফেক্ট।" — এন্ট্রি ৯১-এর "পরের সেশনে আসল করণীয়"-এর ৩টা আইটেমই real-device-এ যাচাই সম্পূর্ণ:

1. ✅ **এন্ট্রি ৯১ ফিক্স ভেরিফায়েড** — POS স্টক-ডিডাকশন (never-load মোডে `base` lookup fallback ফিক্স) real-device-এ বিক্রি + cold-restart করে কনফার্মড, স্টক ঠিকমতো কমছে ও persist থাকছে।
2. ✅ **এন্ট্রি ৮৯-এর ৩টা ফিক্স ভেরিফায়েড** — ক্রয়-এন্ট্রি সেভ (`applyPurchaseBatch()`), self-use ইনভয়েস (`selfUseCost`), self-use কার্ট-টগল (`toggleSelfUse`) — তিনটাই real-device-এ ঠিকমতো কাজ করছে।
3. ✅ **POS On-Demand Cart (`sbm_pos_ondemand_cart`) স্মোক-টেস্ট পাস** — কার্টে পণ্য যোগ/qty বদল, ব্যাচ/মেয়াদ ব্যাজ, self-use টগল, ইনভয়েস সেভ + স্টক-ডিডাকশন — সব ঠিকমতো কাজ করেছে।

**উপসংহার**: never-load মোড (এন্ট্রি ৭৮-৯১-এর পুরো চেইন) এখন real-device-এ পূর্ণাঙ্গ ভেরিফায়েড — বিলিং-ক্রিটিক্যাল সব পাথ (POS বিক্রি, ক্রয়-এন্ট্রি, self-use, on-demand cart) কনফার্মড কাজ করছে। এটা এখন একটা টেস্ট শপে (আপনার ফার্মেসি) আরেকটু সময় ধরে স্বাভাবিক ব্যবহারে রেখে দেখা যেতে পারে, তারপর অন্য দোকানগুলোতেও ধীরে ধীরে (canary rollout) ছড়ানোর কথা ভাবা যায়।

**📁 এই সেশনে যেসব ফাইল বদলেছে**: শুধু `SQLITE_MIGRATION_LOG.md` (এই এন্ট্রি যোগ) — কোনো কোড টাচ হয়নি।

**পরের সেশনে আসল করণীয়** (নতুন কোনো জরুরি বাগ-রিপোর্ট না থাকলে):
1. never-load + POS on-demand cart মোড ফার্মেসি দোকানে কিছুদিন স্বাভাবিক ব্যবহারে রেখে monitor করা (edge case ধরা পড়লে রিপোর্ট করুন)
2. ইচ্ছা হলে অন্য কোনো টেস্ট শপে সতর্কভাবে চালু করে দেখা (canary rollout, ফ্ল্যাগ ডিফল্ট বন্ধই থাকে, তাই প্রতিটা দোকানে আলাদাভাবে সিদ্ধান্ত নেওয়া যায়)
3. বড়-পিকচার আইটেম এখনো বাকি (জরুরি না): Phase 3 (read-path cutover — সব রিড এখনো IndexedDB থেকে), Phase 4 (reconciliation), Phase 5 (পুরনো blob-array কোড অপসারণ, কমপক্ষে ৪-৬ সপ্তাহ স্টেবিলিটি প্রমাণের পর)

---

## 🎯 আগের মাস্টার স্ট্যাটাস (এন্ট্রি ৯১-এ আপডেট — নতুন সেশনে প্রথমে এই সেকশনটাই পড়ুন)
