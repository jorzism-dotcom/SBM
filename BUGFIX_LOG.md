...
### ২০২৬-০৭-৩০ — ক্যাশ উইথড্রয়াল মোডালে "সাপ্লায়ারকে দেওয়া"/"অন্যান্য" ফিল্ড পুরনো এন্ট্রি মনে রাখা
- উপসর্গ (Symptom): "ক্যাশ উইথড্র করুন" পেজ থেকে বের হয়ে আবার ঢুকলে
  "সাপ্লায়ারকে দেওয়া"/"অন্যান্য" ফিল্ডে আগের সাপ্লায়ার নাম/টাকার পরিমাণ/নোট
  থেকে যেত।
- মূল কারণ (Root cause): `cashAmount`/`cashNote`/`cashParty`/`cashType` শুধু
  "বাতিল" বাটন ও "← ড্যাশবোর্ডে ফিরুন" লিংকের ইনলাইন `onClick`-এ রিসেট হতো।
  কিন্তু ফোনের হার্ডওয়্যার ব্যাক বাটন (`useBackHandler(!!cashModal, () => {
  setCashModal(null); return true; })`) শুধু `cashModal`-কে null করত, এই
  ফিল্ডগুলো ছুঁতো না — ফলে ব্যাক বাটন দিয়ে বের হলে স্টেট রয়ে যেত।
- ফিক্স কোথায়: `src/App.jsx`, `Dashboard` কম্পোনেন্ট — নতুন `React.useEffect`
  যেটা `cashModal` falsy হলেই (path নির্বিশেষে) সব উইথড্রয়াল-ফর্ম ইনপুট
  রিসেট করে।
- ব্লাস্ট রেডিয়াস: শুধু UI-লোকাল স্টেট (cashAmount/cashNote/cashParty/
  cashType/cashPartyOpen) — কোনো হিসাব/সিঙ্ক/schema ছোঁয়নি। ফিক্সের আগে সবচেয়ে
  খারাপ কেস: ভুল করে আগের সাপ্লায়ারের নামে/ভুল পরিমাণে উইথড্রয়াল সাবমিট হয়ে
  যাওয়ার ঝুঁকি (ইউজার যদি ফাঁকা ফিল্ড ভেবে নতুন টাকা টাইপ করে কিন্তু পুরনো
  সাপ্লায়ার নাম/টাইপ সিলেক্টেড থেকে যায়)।
- রিগ্রেশন টেস্ট যোগ হয়েছে কি: না — এটা React কম্পোনেন্ট-লোকাল UI স্টেট,
  বর্তমান টেস্ট সুইট (logic/schema/integration/sync) `src/App.jsx` ইম্পোর্ট
  করে না। ম্যানুয়ালি ডিভাইসে যাচাই করা দরকার।
